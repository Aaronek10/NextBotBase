...
if not M.UpdateLadders then
	M.Ladders, M.LaddersUpdate = {}, nil
	function M.UpdateLadders()
		if M.LaddersUpdate and CurTime() - M.LaddersUpdate < 30 then return end
		M.Ladders, M.LaddersUpdate = {}, CurTime()
		local ladders = {}
		for k, v in ipairs(navmesh.GetAllNavAreas()) do
			for _, ladder in ipairs(v:GetLadders()) do
				if not ladders[ladder] then
					ladders[ladder] = true
					M.Ladders[#M.Ladders + 1] = ladder
				end
			end
		end
	end
