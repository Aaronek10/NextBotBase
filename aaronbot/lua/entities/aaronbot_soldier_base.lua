...
--[[------------------------------------
	AaronBot Soldier
	Uses AaronBot Base.
	Follows player if Follower, otherwise neutral.
	Shoots enemies; friendly marks combine as enemies, hostile marks rebels.
	Informs other bots in same group about enemies.
--]]------------------------------------

ENT.Base = "aaronbot_base"
...
ENT.TaskList = {
	["shooting_handler"] = {
		...
		BehaveUpdate = function(self, data, interval)
			...
			if self.IsSeeEnemy and IsValid(enemy) then
				...
				if melee and not self:HasCondition(23) then return end
				if not melee and wep:Clip1() <= 0 then self:WeaponReload() end
				...
				if not shoot then
					...
					if tr.Hit and self:ShouldBeEnemy(tr.Entity) then shoot = true end
				end
				if shoot then
			...
			elseif not melee and wep:Clip1() < wep:GetMaxClip1() / 2 then
				self:WeaponReload()
			end
		end,
		...
		OnStart = function(self, data)
			...
			self.UpdateEnemyHandler = function(forceupdateenemies)
				...
				if IsValid(newenemy) then
					...
					if data.HasEnemy then self:RunTask("EnemyLost", prevenemy) end
					data.HasEnemy = false
					self.IsSeeEnemy = false
				end
	...
	["movement_handler"] = {
		OnStart = function(self, data)
			...
			local findwep = not self:HasWeapon() and self:FindWeapon()
			...
			elseif findwep then
				...
				if IsValid(self.Target) then
					if self:GetRangeSquaredTo(self.Target) > 100 * 100 or not self:CanSeePosition(self.Target) then
	...
	["movement_getweapon"] = {
		...
		BehaveUpdate = function(self, data)
			...
			if result then
				...
				self:StartTask("movement_wait")
				if self:GetRangeSquaredTo(data.Wep) < 50 * 50 then self:SetupWeapon(data.Wep) end
	...
	["movement_followtarget"] = {
		BehaveUpdate = function(self, data)
			...
			if not data.Pos or self.Target:GetPos():DistToSqr(data.Pos) > 50 * 50 then
			...
			local result = self:ControlPath(not self.IsSeeEnemy)
			if result then
				self:TaskComplete("movement_followtarget")
				self:StartTask("movement_wait")
			elseif result == false then
	...
	["movement_followenemy"] = {
		OnStart = function(self, data)
			...
			data.Walk = not self:CanSeePosition(data.Pos) and self:GetRangeSquaredTo(data.Pos) < 2000 * 2000
			if not self:PathIsValid() then
		...
		BehaveUpdate = function(self, data)
			...
			if IsValid(self.Target) and (not self:IsMeleeWeapon() or self.Target:GetPos():DistToSqr(newpos) > 300 * 300) then
				self:TaskFail("movement_followenemy")
		...
		ShouldWalk = function(self, data)
			if data.Walk then return true end
		end,
	},
	["movement_randomwalk"] = {
		OnStart = function(self, data)
			local pos = self:GetRandomWalkPosition()
			if pos then self:SetupPath(pos) else self:GetPath():Invalidate() end
			if not self:PathIsValid() then
		...
		ShouldWalk = function(self, data) return true end,
	},
	["movement_custompos"] = {
		OnStart = function(self, data)
	...
	["movement_meleecontrol"] = {
		BehaveUpdate = function(self, data)
			if not self:IsMeleeWeapon() or self:IsTaskActive("movement_followenemy") then return end
			if IsValid(self:GetEnemy()) and (not IsValid(self.Target) or self.Target:GetPos():DistToSqr(self:GetEnemy():GetPos()) < 300 * 300) then
				self:TaskFail("movement_wait")
				self:TaskFail("movement_followtarget")
				self:TaskFail("movement_randomwalk")
				self:TaskFail("movement_custompos")
	...
	["inform_handler"] = {
		...
		BehaveUpdate = function(self, data, interval)
			...
			if self.IsSeeEnemy and (not data.EnemyPosInform or CurTime() >= data.EnemyPosInform) then
...
ENEMY_CLASSES = {
	npc_crow = ENEMY_NEUTRAL, npc_monk = ENEMY_FRIENDLY, npc_pigeon = ENEMY_NEUTRAL, npc_seagull = ENEMY_NEUTRAL,
	npc_combine_camera = ENEMY_NEUTRAL, npc_cscanner = ENEMY_HOSTILE, npc_combinegunship = ENEMY_HOSTILE,
	npc_combine_s = ENEMY_HOSTILE, npc_helicopter = ENEMY_HOSTILE, npc_manhack = ENEMY_HOSTILE,
	npc_metropolice = ENEMY_HOSTILE, npc_rollermine = ENEMY_HOSTILE, npc_clawscanner = ENEMY_HOSTILE,
	npc_stalker = ENEMY_HOSTILE, npc_strider = ENEMY_HOSTILE, npc_turret_floor = ENEMY_HOSTILE, npc_sniper = ENEMY_HOSTILE,
	npc_alyx = ENEMY_FRIENDLY, npc_barney = ENEMY_FRIENDLY, npc_citizen = ENEMY_FRIENDLY, npc_dog = ENEMY_FRIENDLY,
	npc_kleiner = ENEMY_FRIENDLY, npc_mossman = ENEMY_FRIENDLY, npc_eli = ENEMY_FRIENDLY, npc_gman = ENEMY_NEUTRAL,
	npc_odessa = ENEMY_FRIENDLY, npc_vortigaunt = ENEMY_FRIENDLY, npc_magnusson = ENEMY_FRIENDLY, npc_breen = ENEMY_NEUTRAL,
	npc_antlion = ENEMY_MONSTER, npc_antlionguard = ENEMY_MONSTER, npc_barnacle = ENEMY_MONSTER,
	npc_headcrab_fast = ENEMY_MONSTER, npc_fastzombie = ENEMY_MONSTER, npc_fastzombie_torso = ENEMY_MONSTER,
	npc_headcrab = ENEMY_MONSTER, npc_headcrab_poison = ENEMY_MONSTER, npc_headcrab_black = ENEMY_MONSTER,
	npc_poisonzombie = ENEMY_MONSTER, npc_zombie = ENEMY_MONSTER, npc_zombie_torso = ENEMY_MONSTER,
	npc_antlion_grub = ENEMY_MONSTER, npc_antlionguardian = ENEMY_MONSTER, npc_antlion_worker = ENEMY_MONSTER,
	npc_zombine = ENEMY_MONSTER, npc_hunter = ENEMY_HOSTILE,
	monster_alien_grunt = ENEMY_MONSTER, monster_alien_slave = ENEMY_MONSTER, monster_human_assassin = ENEMY_HOSTILE,
	monster_babycrab = ENEMY_MONSTER, monster_bullchicken = ENEMY_MONSTER, monster_alien_controller = ENEMY_MONSTER,
	monster_gargantua = ENEMY_MONSTER, monster_bigmomma = ENEMY_MONSTER, monster_human_grunt = ENEMY_HOSTILE,
	monster_headcrab = ENEMY_MONSTER, monster_cockroach = ENEMY_NEUTRAL, monster_turret = ENEMY_HOSTILE,
	monster_houndeye = ENEMY_MONSTER, monster_miniturret = ENEMY_HOSTILE, monster_nihilanth = ENEMY_MONSTER,
	monster_scientist = ENEMY_FRIENDLY, monster_barney = ENEMY_FRIENDLY, monster_sentry = ENEMY_HOSTILE,
	monster_snark = ENEMY_MONSTER, monster_tentacle = ENEMY_MONSTER, monster_zombie = ENEMY_MONSTER,
}
	...
	hook.Add("OnEntityCreated", self, function(self, ent)
		if ent:GetClass() == "npc_sniper" then
			timer.Simple(0, function()
				if IsValid(self) and IsValid(ent) then self:SetupEntityRelationship(ent) end
			end)
		else
...
function ENT:SetupEntityRelationship(ent)
	...
	if stdd then
		...
		if ent:IsNPC() then ent:AddEntityRelationship(self, d) end
	end
end
	...
	if not self:IsControlledByPlayer() and not self:DisableBehaviour() then
		...
		if blocker then
			...
			if self:HasWeapon() and class:StartWith("func_breakable") then
				self:WeaponPrimaryAttack()
			elseif class == "prop_door_rotating" and blocker:GetInternalVariable("m_eDoorState") != 2 and (not self.OpenDoorTime or CurTime() - self.OpenDoorTime > 2) then
				self.OpenDoorTime = CurTime()
				blocker:Fire("Use")
			elseif class == "func_door_rotating" and blocker:GetInternalVariable("m_toggle_state") == 1 and (not self.OpenDoorTime or CurTime() - self.OpenDoorTime > 2) then
				self.OpenDoorTime = CurTime()
				blocker:Fire("Use")
			elseif class == "func_door" and blocker:GetInternalVariable("m_toggle_state") == 1 and (not self.OpenDoorTime or CurTime() - self.OpenDoorTime > 2) then
				self.OpenDoorTime = CurTime()
				blocker:Fire("Use")
			end
		end
...
function ENT:OnTouch(ent, trace)
	if not self.OnContactAllowed then return end
	local vel = ent:GetVelocity()
	if not vel:IsZero() and self:GetRelationship(ent) != D_HT then
...
function ENT:FindWeapon()
	...
	for k, v in ipairs(ents.GetAll()) do
		...
		if r > searchrange or not self:CanPickupWeapon(v) or not self:CanSeePosition(v) then continue end
		local w = v:GetWeight()
		if not wep or w > weight and r - range < distlimit or w < weight and r - range > distlimit or w == weight and r < range then
...
function ENT:GetDesiredEnemyRelationship(ent, stdd)
	...
	if stdd == ENEMY_MONSTER and not efr then return D_HT end
...
function ENT:EntityIsFriendly(ent)
	if ent:IsNPC() then
		...
		if (ent:GetClass() == "npc_antlion" or ent:GetClass() == "npc_antlion_worker") and game.GetGlobalState("antlion_allied") == GLOBAL_ON then return true end
		if ent:GetClass() == "npc_sniper" and IsValid(self.Target) and ent:Disposition(self.Target) == D_LI then return true end
	end
...
function ENT:ShouldBeEnemy(ent)
	...
	if class == "npc_strider" or class == "npc_combinegunship" or class == "npc_helicopter" or class == "npc_combinedropship" then
		if not self:HasWeapon() or self:GetActiveWeapon():GetClass() != "weapon_rpg" then return false end
	end
...
function ENT:ShootBlocker(start, pos, filter)
	local tr = util.TraceHull({start = start, endpos = pos, filter = filter, mask = MASK_SHOT, mins = Vector(-2, -2, -2), maxs = Vector(2, 2, 2)})
...
function ENT:GetRandomWalkPosition()
	...
	if self:UsingNodeGraph() then
		local cur = AaronBotNodeGraph and AaronBotNodeGraph.GetNearestNode and AaronBotNodeGraph.GetNearestNode(pos)
		...
		while not table.IsEmpty(opened) do
			...
			opened[nodeid] = nil
			closed[nodeid] = true
			local node = AaronBotNodeGraph.GetNodeByID(nodeid)
			if costs[nodeid] >= destlen then return node:GetOrigin() end
			for k, v in ipairs(node:GetAdjacentNodes()) do
				if not closed[v:GetID()] then
					costs[v:GetID()] = costs[nodeid] + v:GetOrigin():DistToSqr(node:GetOrigin())
					opened[v:GetID()] = true
				end
			end
		end
		...
		while not table.IsEmpty(opened) do
			...
			if costs[areaid] >= destlen then return area:GetRandomPoint() end
			for k, v in ipairs(area:GetAdjacentAreas()) do
				if not closed[v:GetID()] then
					costs[v:GetID()] = costs[areaid] + v:GetCenter():DistToSqr(area:GetCenter())
					opened[v:GetID()] = true
				end
			end
		end
...
function ENT:SetupTaskList(list)
	...
	for k, v in pairs(self.TaskList) do list[k] = v end
end
