...
ENT.PrintName = "AaronBot Soldier (Friendly)"

list.Set("NPC", "aaronbot_soldier_friendly", {
	Name = ENT.PrintName,
	Class = "aaronbot_soldier_friendly",
	Category = "AaronBot",
	Weapons = {"weapon_smg1", "weapon_ar2", "weapon_shotgun", "weapon_pistol", "weapon_357", "weapon_crossbow"},
})

if CLIENT then
	language.Add("aaronbot_soldier_friendly", "AaronBot Soldier (Friendly)")
	return
end

cvars.AddChangeCallback("aaronbot_soldier_playerdisposition", function(cvar, old, new)
	local disposition = tonumber(new)
	for k, v in ipairs(ents.FindByClass("aaronbot_soldier_friendly")) do
...
function ENT:Initialize()
	...
	self:SetClassRelationship("aaronbot_soldier_friendly", D_LI)
	self:SetClassRelationship("aaronbot_soldier_hostile", D_HT)
