...
if SERVER then
	util.AddNetworkString("weapon_pistol_aaronbot.muzzleflash")
else
	killicon.AddFont("weapon_pistol_aaronbot", "HL2MPTypeDeath", "-", Color(255, 80, 0))
end
...
SWEP.Weight = 2
SWEP.Primary = {Ammo = "Pistol", ClipSize = 18, DefaultClip = 18}
...
function SWEP:PrimaryAttack()
	...
	self:FireBullets({Num = 1, Src = owner:GetShootPos(), Dir = owner:GetAimVector(), Spread = vector_origin, Distance = MAX_TRACE_LENGTH, AmmoType = self:GetPrimaryAmmoType(), Damage = 5, Force = 1, Attacker = owner})
	self:DoMuzzleFlash()
...
function SWEP:DoMuzzleFlash()
	if SERVER then
		net.Start("weapon_pistol_aaronbot.muzzleflash", true)
...
if CLIENT then
	net.Receive("weapon_pistol_aaronbot.muzzleflash", function()
...
function SWEP:GetNPCBurstSettings() return 1, 3, 0.5 end
