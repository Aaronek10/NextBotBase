...
if SERVER then
	util.AddNetworkString("weapon_shotgun_aaronbot.muzzleflash")
else
	killicon.AddFont("weapon_shotgun_aaronbot", "HL2MPTypeDeath", "0", Color(255, 80, 0))
end
...
SWEP.Weight = 4
SWEP.Primary = {Ammo = "Buckshot", ClipSize = 6, DefaultClip = 6}
...
function SWEP:PrimaryAttack()
	...
	self:FireBullets({Num = 8, Src = owner:GetShootPos(), Dir = owner:GetAimVector(), Spread = VECTOR_CONE_10DEGREES, Distance = MAX_TRACE_LENGTH, AmmoType = self:GetPrimaryAmmoType(), Damage = 8, Force = 1, Attacker = owner})
	self:DoMuzzleFlash()
...
function SWEP:DoMuzzleFlash()
	if SERVER then
		net.Start("weapon_shotgun_aaronbot.muzzleflash", true)
...
if CLIENT then
	net.Receive("weapon_shotgun_aaronbot.muzzleflash", function()
...
function SWEP:SecondaryAttack()
	...
	local owner = self:GetOwner()
	self:FireBullets({Num = 12, Src = owner:GetShootPos(), Dir = owner:GetAimVector(), Spread = VECTOR_CONE_10DEGREES, Distance = MAX_TRACE_LENGTH, AmmoType = self:GetPrimaryAmmoType(), Damage = 8, Attacker = owner})
	self:DoMuzzleFlash()
...
function SWEP:GetNPCBurstSettings() return 1, 3, 0.7 end
function SWEP:GetNPCRestTimes() return 1.2, 1.5 end
